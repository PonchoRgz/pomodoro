...
// Format time to display weeks, days, hours, minutes, and seconds
...
function updateDisplay() {
    ...
    document.title = isRunning ? `(${formattedTime}) Pomodoro Timer` : 'Pomodoro Timer';
}
...
function snoozeTimer() {
    ...
    snoozeModal.classList.add('show');
}
...
function handleSnoozeSelection(minutes) {
    timeLeft += minutes * 60;
    updateDisplay();
    snoozeModal.classList.remove('show');
}
...
// Add event listeners for snooze options
snoozeOptions.forEach(option => {
    option.addEventListener('click', () => {
        const minutes = parseInt(option.dataset.minutes);
        handleSnoozeSelection(minutes);
    });
});
